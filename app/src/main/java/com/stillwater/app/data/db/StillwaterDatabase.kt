 ...
 * event_trigger — added tables only, so an auto-migration suffices.
 * Exported schema JSON lives in app/schemas/ (checked in) so migrations get
 * real tests. Trigger seeding happens idempotently on open (see DataModule).
 */
@Database(
    entities = [
